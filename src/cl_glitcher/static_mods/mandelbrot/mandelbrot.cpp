...
void jsonObjExecute(JsonObjExecuteParams const & params)
{
  ...
  auto stepsArr = lineSegmentToValueArray(data.steps, imageCount);
  auto nArr = lineSegmentToValueArray(data.n, imageCount);
  auto radiusArr = lineSegmentToValueArray(data.radius, imageCount);
  auto sphereOffsetXArr = lineSegmentToValueArray(data.sphereOffsetX, imageCount);
  auto sphereOffsetYArr = lineSegmentToValueArray(data.sphereOffsetY, imageCount);
  auto sphereOffsetZArr = lineSegmentToValueArray(data.sphereOffsetZ, imageCount);
  ...
  imageNameLoop(
    ...
    [&](std::string const & outputFilename) {
      std::cout << stepsArr[i] << " " << nArr[i] << " " << radiusArr[i] << "\n";

      MandelbrotProgram program {params.gpuHandle};
      program.execute(
        params.commandQueue,
        params.gpuHandle,
        params.currentImage,
        ...
        radiusArr[i],
        data.imageFeedback,
        {sphereOffsetXArr[i], sphereOffsetYArr[i], sphereOffsetZArr[i]});
